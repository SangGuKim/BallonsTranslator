...
런타임 설정 파일은 저장소 루트 기준 `resources/font_registry.json`에 둔다. 이 파일은
...
registry = build_font_registry(
    ...
    locale="ko-KR",
    font_registry_config_path="resources/font_registry.json",
)
```
...
python font-system-lab/tools/probe_font_registry_logic.py \
  --font-registry-config resources/font_registry.json \
...
런타임 경로는 `resources/font_registry.json` 하나만 사용한다.
